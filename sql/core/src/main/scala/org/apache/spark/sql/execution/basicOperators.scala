...
case class PartitionProject(projectList: Seq[Expression], child: SparkPlan) extends UnaryNode {
  ...
  def generateIterator(input: Iterator[Row]): Iterator[Row] = {
    ...
    // IMPLEMENT ME
    val parts: Iterator[DiskPartition] = DiskHashedRelation(input, keyGenerator).getIterator()
	...
	//Assign it to the iterator generated by CS143Utils
	if (parts.hasNext) {
	    val data: Iterator[Row] = parts.next().getData()
	    if(data.hasNext) {
		it = CS143Utils.generateCachingIterator(projectList, child.output)(data) 
		true
	    }
	    else {
		fetchNextPartition()
